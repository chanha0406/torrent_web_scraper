...
class ScraperTemplate(metaclass=ABCMeta):
    def __init__(
        ...
        self.__scraper_config = ScraperConfig(
            self.name, self.__scraper_configuration_file, self.__local_machine_status_file
        )
        ...
        self.__transmission_delegate = TransmissionDelegate(
            trans_id, trans_pw, trans_host, trans_port, self.history_delegate
        )
    ...
    def aggregation_categories(self):
        ...
        categories = [
            x.strip() for x in self.__scraper_config.get_config_scraper("categories").split(",")
        ]
    ...
    @abstractmethod
    def parse_magnets_from_page_url(self, url, item):
        pass
    ...
    def execute_scraper(self):
        if self.name == "bt4g":
            self.__execute_scraper_for_query()
        else:
            for category in self.__categories:
                self.__execute_scraper_for_category(category)

    def __execute_scraper_for_query(self):
        base_url = self.__scraper_config.get_base_url()

        for item in title_list:
            magnets = self.parse_magnets_from_page_url(base_url, item)
